...
static int mxl862xx_cpu_bridge_port_id(struct dsa_switch *ds, int port)
{
	...
	if (priv->tag_8021q_setup_done && p->bridge_port_cpu)
...
static int mxl862xx_setup_link_local_trap(struct dsa_switch *ds, int port)
{
	...
	if (priv->tag_8021q_setup_done && p->cpu_egress_evlan.in_use) {
...
static void mxl862xx_teardown_tag_8021q(struct dsa_switch *ds)
{
	...
	priv->tag_8021q_setup_done = false;
}
 ...
 * @ds: DSA switch
 *
 * Called once all user ports have been set up (each has a non-zero FID
 * from add_single_port_bridge). Sets up virtual bridge ports for each
 * user port and registers the tag_8021q framework, which triggers
 * tag_8021q_vlan_add callbacks to install management VIDs.
 */
static int mxl862xx_complete_tag_8021q_setup(struct dsa_switch *ds)
{
	...
	priv->tag_8021q_setup_done = true;

	/* In tag_8021q mode TX goes through the bridge engine (CTP
	 * ingress EVLAN reassigns to a virtual bridge port), so
	 * unknown unicast and multicast must be flooded for frames
	 * from the CPU to reach user ports.  The per-port bridges
	 * may have been created with flooding disabled (SpTag mode
	 * default), so update them now.
	 */
	dsa_switch_for_each_user_port(dp, ds) {
		int port = dp->index;
	...
	/* Portmaps and link-local traps were installed during port_setup
	 * before tag_8021q was active, so they still target the physical
	 * CPU port.  Now that virtual bridge ports exist, refresh them.
	 */
	return mxl862xx_refresh_cpu_targets(ds);
}
					...
					enum dsa_tag_protocol proto)
{
	...
	case DSA_TAG_PROTO_MXL862_8021Q:
		/* During initial setup, port_setup hasn't run yet, so
		 * defer the real work to complete_tag_8021q_setup().
		 */
		if (!ds->setup)
			return 0;
...
static int mxl862xx_port_setup(struct dsa_switch *ds, int port)
{
	...
	if (is_cpu_port) {
		/* Initialize CPU port VF and EVLAN state for tag_8021q.
		 * The ingress EVLAN block is pre-allocated at max size
		 * to avoid firmware table fragmentation.
		 * In native SpTag mode these stay unused (in_use=false).
		 */
	 ...
	 * user ports. Blocks are allocated once and kept for the
	 * lifetime of the port to avoid firmware EVLAN table
	 * fragmentation (the firmware allocator cannot defragment).
	 * CPU ports do not use EVLAN or VF -- frames pass through
	 * without processing.
	 */
	mxl862xx_evlan_block_init(&priv->ports[port].ingress_evlan,
	...
	/* Deferred tag_8021q setup: after the last user port is
	 * initialized, complete the tag_8021q setup that was deferred
	 * from change_tag_protocol() (which runs before port_setup).
	 *
	 * Check whether all user ports have been set up by counting
	 * those with a non-zero fid (assigned by add_single_port_bridge).
	 */
	if (priv->tag_proto == DSA_TAG_PROTO_MXL862_8021Q &&
	    !priv->tag_8021q_setup_done) {
		bool all_ready = true;

		dsa_switch_for_each_user_port(dp, ds) {
			if (!priv->ports[dp->index].fid) {
				all_ready = false;
				break;
			}
		}
				    ...
				    struct dsa_db db)
{
	...
	if (dsa_is_cpu_port(ds, port) && priv->tag_8021q_setup_done &&
	    db.type == DSA_DB_PORT) {
					...
					const struct dsa_bridge *bridge)
{
	...
	struct mxl862xx_priv *priv = ds->priv;
	u16 fid = (u16)(uintptr_t)bridge->priv;
	struct dsa_port *member_dp;
				 ...
				 const unsigned char *addr, u16 vid, struct dsa_db db)
{
	...
	if (priv->tag_8021q_setup_done && dsa_is_cpu_port(ds, port) &&
				 ...
				 const unsigned char *addr, u16 vid, struct dsa_db db)
{
	...
	if (priv->tag_8021q_setup_done && dsa_is_cpu_port(ds, port) &&
		...
		if (target_dp->bridge && target_dp->bridge->priv)
			mxl862xx_fdb_del_per_fid(ds, addr, vid,
						 (u16)(uintptr_t)target_dp->bridge->priv);
	}
				 ...
				 struct dsa_db db)
{
	 ...
	 */
	if (priv->tag_8021q_setup_done && db.type == DSA_DB_PORT)
